...
def process_images_yolo(args, imgs_array, yolo_model):
    """Process images with YOLO detector and return bboxes."""
    # bboxes_batch = []
    
    # for img in imgs_array:
    #     # YOLO expects BGR format (numpy array)
    #     results = yolo_model(img, conf=args.bbox_thr, verbose=False)
        
    #     bboxes = []
    #     for result in results:
    #         # Only person class (class_id=0)
    #         for box in result.boxes:
    #             if int(box.cls) == 0:  # person class in COCO is 0
    #                 x1, y1, x2, y2 = box.xyxy[0].cpu().numpy()
    #                 bboxes.append([x1, y1, x2, y2])
        
    #     bboxes_batch.append(np.array(bboxes) if bboxes else np.array([]))
    
    # return bboxes_batch
    assert imgs_array.ndim == 4  # (B,H,W,C)
    ...
    device_id = 0 if "cuda" in str(args.device).lower() else "cpu"
    t0 = time.time()
    ...
    print("one yolo:", time.time()-t0)


    bboxes_batch = []
    for r in results:
        if r.boxes is None or len(r.boxes) == 0:
            bboxes_batch.append(np.array([]))
            continue
        xyxy = r.boxes.xyxy.detach().cpu().numpy()  # (N,4)
        conf = r.boxes.conf.detach().cpu().numpy()
        if args.max_num is not None and args.max_num > 0 and len(xyxy) > args.max_num:
            keep_idx = np.argsort(conf)[::-1][:args.max_num]
            xyxy = xyxy[keep_idx]

        bboxes_batch.append(xyxy)
...
def img_save_and_vis(
    img, results, output_path, input_shape, heatmap_scale, kpt_colors, kpt_thr, radius, skeleton_info, thickness, max_num=-1
):
    ...
    if max_num > 0 and len(person_data) > max_num:
        person_data = sorted(person_data, key=lambda x: x['score'], reverse=True)[:max_num]
    
    for person in person_data:
        instance_keypoints.append(person['keypoints'])
        instance_scores.append(person['keypoint_scores'])
...
def main():
    ...
    parser.add_argument("--yolo-model", default="yolov8m.pt", help="YOLO model (yolov8n/s/m/l/x)")
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Maximum number of persons to visualize. -1 means all persons."
    )

    args = parser.parse_args()

    use_det = args.yolo_model is not None and args.yolo_model != ""
    if use_det:
        assert has_yolo, "Please install ultralytics: pip install ultralytics"
    ...
    print(f"Total {len(image_names)} images to process.")
    ...
    for batch_idx, (batch_image_name, batch_orig_imgs, batch_imgs) in tqdm(
        ...
        if use_det:
            bboxes_batch = process_images_yolo(args, batch_orig_imgs.numpy(), yolo_model)
        else:
            bboxes_batch = [[] for _ in range(len(batch_orig_imgs))]
    ...
    print("Finished pose estimation on all images.")
